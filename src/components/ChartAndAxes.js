...
const WeatherBlanket = ({ blanketData }) => {
    ...
    function storeMousePosition(e) {
        ...
        const reversedYears = [
            ...toolbarParams.selectedYears.slice().reverse(),
        ];
        ...
        const dayData =
            blanketData?.[reversedYears[yearIndex]]?.days?.[dayIndex];
...
const BlanketTooltip = ({ mouseCoords, data }) => {
    ...
    const precipitation = data['PRCP'] == 9999 ? 0 : data['PRCP'];
    ...
    useEffect(() => {
        if (tooltipRef.current) {
            ...
            const left =
                mouseCoords[0] + width > screenWidth
                    ? mouseCoords[0] - width
                    : mouseCoords[0];
            let top;
    ...
    return (
        ...
        >
            <div className="tooltip-wrapper">
                <h3> {data['date']} </h3>
                <ul>
                    <li>High Temp: {data['TMAX']}&deg;F</li>
                    <li>Low Temp: {data['TMIN']}&deg;F</li>
